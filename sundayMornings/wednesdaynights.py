...
wednesdayWorldviewDateInputKey = 'bd972ee4-63e5-4b3b-8f33-fc07b37bad3d'
...
def startStream(promptForConfirmation = True):
    ...
    try:
        ...
        if vFunc.vMixFunction(function='SnapshotInput',
                              value=quote("D:\Downloads\Thumbnail - {0:dd MMM yyyy} - WW.jpg", safe='.'),
                              rawInputKey=wednesdayWorldviewDateInputKey) \
                is False: error(previousLines=3)

        # runs ptz.recallPreset asynchronously
        recallPresetThread = threading.Thread(target=ptz.recallPreset, args=([PTZ_PREACHING_PRESET]), kwargs={})
...
def endStream():
    """
    Fade to the slides input, send the date slide to vMix preview, stops recording,
    prompts (optionally) to stop streaming/external output.
...
def test():
    print('Wednesday Worldview "test()" just called')
    return True
